...
export const getAllPaymentsByUser = async(req: Request, res: Response) => {
    try {
        const {id} = req.body;
        const payments = await prisma.payment.findMany({where: {customerId: id}, include: { PaymentDetail: true }});
        res.json({
            msg: 'ok',
            error: false,
...
export const getPaymentById = async(req: Request, res: Response) => {
    try {
        ...
        const existingPayment = await prisma.payment.findFirst({where: {id: idNumber}, include: { PaymentDetail: true }});
